...
if menu == "Insight Conversation":
    ...
    if prompt := st.chat_input("Ask me about your data! (e.g., 'What were the total number of reviews per month?')"):
        ...
        if "total number of reviews per month" in prompt.lower():
            monthly_reviews = df_filtered.groupby(['month_year', 'category'], as_index=False)['reviews'].sum().drop_duplicates()
            messages = [
                {"role": "user", "content": f"Based on the data:\n{monthly_reviews.to_string()}\nProvide a friendly and concise summary of total reviews per month. Example: 'Hey! January 2025 had 3000 reviews for Toothbrush!'"}
            ]
            response = client.chat.completions.create(model="gpt-4o", messages=messages)
            reply = response.choices[0].message.content

            st.session_state.messages_insight.append({"role": "assistant", "content": reply})
            with st.chat_message("assistant"):
                st.write(reply)
            ...
            if len(months) >= 2:
                ...
                messages = [
                    {"role": "user", "content": f"Compare reviews for {month1} 2025 ({month1_reviews}) and {month2} 2025 ({month2_reviews}). Example: 'Hey! {month1} 2025 had {month1_reviews} reviews, compared to {month2_reviews} in {month2} 2025!'"}
                ]
                response = client.chat.completions.create(model="gpt-4o", messages=messages)
                reply = response.choices[0].message.content
                ...
                with st.chat_message("assistant"):
                    ...
                    fig = go.Figure(data=[go.Bar(x=[month1 + " 2025", month2 + " 2025"], y=[month1_reviews, month2_reviews], marker_color=['#FF6B6B', '#4ECDC4'])])
                    fig.update_layout(
            ...
            messages = [
                {"role": "user", "content": f"Compare reviews for last month ({last_month_reviews}) and this month ({this_month_reviews}). Example: 'Hey! Last month had {last_month_reviews} reviews, while this month has {this_month_reviews} for the toothbrush category!'"}
            ]
            response = client.chat.completions.create(model="gpt-4o", messages=messages)
            reply = response.choices[0].message.content
            ...
            metric = next((col for col in df.columns if any(keyword in col.lower() for keyword in ["sales", "sale"])), "reviews")
            if metric == "reviews":
            ...
            messages = [
                {"role": "user", "content": f"Identify the {entity} with the most and least {metric} per month from:\n{entity_metrics.to_string()}. Example: 'In January 2025, SKU 1 had the most reviews with 3000, and SKU 2 had the least with 5.'"}
            ]
            response = client.chat.completions.create(model="gpt-4o", messages=messages)
            reply = response.choices[0].message.content
            ...
            messages = [
                {"role": "user", "content": "I don’t understand. Suggest: 'What were the total number of reviews per month?' or 'Upload a CSV file!'"}
            ]
            response = client.chat.completions.create(model="gpt-4o", messages=messages)
            reply = response.choices[0].message.content
        ...
        if df.empty:
            ...
            if "products are out of stock" in prompt.lower() and "how many" in prompt.lower():
                ...
                if out_of_stock_count > 0:
                    ...
                    messages = [
                        {"role": "user", "content": f"List out-of-stock products with titles and quantities (0). Example: 'Hey! We’ve checked your stock, and here are the products out of stock: [list]. Time to restock!'"}
                    ]
                    response = client.chat.completions.create(model="gpt-4o", messages=messages)
                    reply = response.choices[0].message.content

                    st.session_state.messages_shopify.append({"role": "assistant", "content": reply})
                    with st.chat_message("assistant"):
                        st.write(reply)
                        st.write("### Analysis Results")
                    ...
                    messages = [
                        {"role": "user", "content": "No out-of-stock products. Example: 'Hey! No products are out of stock right now!'"}
                    ]
                    response = client.chat.completions.create(model="gpt-4o", messages=messages)
                    reply = response.choices[0].message.content
                ...
                messages = [
                    {"role": "user", "content": f"Compare product updates for last month ({last_month_count}) and this month ({this_month_count}). Example: 'Hey! Last month had {last_month_count} updates, while this month has {this_month_count} for {category or 'all'}!'"}
                ]
                response = client.chat.completions.create(model="gpt-4o", messages=messages)
                reply = response.choices[0].message.content
                ...
                messages = [
                    {"role": "user", "content": "Unclear question. Suggest: 'Which products are out of stock?' or 'How many products were updated last month?'"}
                ]
                response = client.chat.completions.create(model="gpt-4o", messages=messages)
                reply = response.choices[0].message.content
